...
static gboolean
gvfs_udisks2_volume_is_encrypted_file_container_class (GVfsUDisks2Volume *volume)
{
  UDisksClient *client;
  UDisksLoop *loop;
  UDisksObject *object;
  ...
  /* Check if the volume is an encrypted loop device */
  loop = udisks_client_get_loop_for_block (client, volume->block);
  if (loop != NULL)
    {
      object = UDISKS_OBJECT (g_dbus_interface_get_object (G_DBUS_INTERFACE (volume->block)));
      if (object != NULL && udisks_object_get_encrypted (UDISKS_OBJECT (object)) != NULL)
        return TRUE;
    }
  ...
  if (strcmp (kind, G_VOLUME_IDENTIFIER_KIND_CLASS) == 0)
    {
      ...
      else if (gvfs_udisks2_volume_is_encrypted_file_container_class (volume))
        ret = g_strdup ("encrypted-file-container");
      else
        ret = g_strdup ("device");
    }
