  ...
  for (l = parent_ids; l != NULL; l = l->next)
    {
      ...
      if (source_id_property && parent_id_property && (g_strcmp0 (parent_id_property, parent_id) == 0))
        {
  ...
  for (ll = parent_ids; ll != NULL; ll = ll->next)
    {
      ...
      if ((volatile_entry_id = get_volatile_entry_id (self, entry, k)) != NULL)
        ...
        g_debug ("  remove_entry: Removed (%s, %s) -> %p, %d\n", title, parent_id, entry, ((GObject *) entry)->ref_count);
                                     ...
                                     const gchar          *filename)
{
   ...
   * for the folder too.
   *
   * FIXME: Below fix is hack-ish and will fail if we wish to create a folder
