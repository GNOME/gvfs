                   ...
                   gboolean           track_dir_collisions)
{
  ...
  for (l = parent_ids; l != NULL; l = l->next)
    {
      ...
      if (old_entry != NULL && g_strcmp0 (gdata_entry_get_id (old_entry), id) != 0)
        {
          if ((e = g_list_find (self->dir_collisions, old_entry)) == NULL)
            {
      ...
      if (insert_title)
      ...
      else
        {
          if (track_dir_collisions)
            {
              if ((e = g_list_find (self->dir_collisions, entry)) == NULL)
                {
  ...
  return insert_title && (!source_id_property ||  !volatile_entry);
}
