...
gvfs_google_test_init (struct GVfsGoogleTestClass **_self, GError **error)
{
  ...
  *_self = g_malloc (sizeof (struct GVfsGoogleTestClass));
  ...
  if (child_error != NULL && child_error->code != G_IO_ERROR_EXISTS)
    {
      g_propagate_error (error, g_steal_pointer (&child_error));
      return;
    }
  ...
  info = g_file_query_info (test_dir,
  ...
  test_dir_id = g_file_info_get_attribute_as_string (info, G_FILE_ATTRIBUTE_ID_FILE);
  ...
  g_clear_object (&info);
  info = g_file_query_info (test_dummy_file,
                            G_FILE_ATTRIBUTE_ID_FILE,
                            G_FILE_QUERY_INFO_NONE,
                            NULL,
                            &child_error);
  ...
  test_dummy_file_id = g_file_info_get_attribute_as_string (info, G_FILE_ATTRIBUTE_ID_FILE);
...
gvfs_google_test_copy_file_from_one_parent_to_other_simple (gconstpointer user_data)
{
  ...
  parent_path = g_file_get_path (self->test_dir);
  copy_test_dir = g_file_new_build_filename (parent_path, OP_COPY_TEST_DIRECTORY, NULL);

  op_done = delete_and_make_new_directory (copy_test_dir, &error);
  g_assert_no_error (error);
  g_assert_true (op_done);

  g_clear_object (&copy_test_dir);
  copy_test_dir = g_file_get_child_for_display_name (self->test_dir, OP_COPY_TEST_DIRECTORY, &error);
  g_assert_no_error (error);
  g_assert_nonnull (copy_test_dir);

  info = g_file_query_info (copy_test_dir,
                            G_FILE_ATTRIBUTE_ID_FILE","
  ...
  copy_test_dir_id = g_file_info_get_attribute_as_string (info, G_FILE_ATTRIBUTE_ID_FILE);
  g_message ("Test Dir ID: %s", copy_test_dir_id);
...
static void
gvfs_google_test_copy_file_inside_same_parent_simple (gconstpointer user_data)
{
  struct GVfsGoogleTestClass *self = (struct GVfsGoogleTestClass *) user_data;
  gboolean op_done;
  g_autoptr(GError) error = NULL;
...
main (gint argc, gchar *argv [])
{
  ...
  g_test_add_data_func ("/copy_file/different_parents_simple_copy",
                        (gconstpointer) self,
                        gvfs_google_test_copy_file_from_one_parent_to_other_simple);

  g_test_add_data_func ("/copy_file/same_parents_simple_copy",
                        (gconstpointer) self,
                        gvfs_google_test_copy_file_inside_same_parent_simple);
