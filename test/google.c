...
static GFile *
create_duplicate_file_with_random_title (GFile *source_file, GError **error)
{
  gboolean op_done = FALSE;
   ...
   * this random string as the title of the destination file. */
  ...
  op_done = g_file_copy (source_file, dummy_dest_file, G_FILE_COPY_NONE, NULL, NULL, NULL, &child_error);
  if (child_error != NULL)
    {
      g_propagate_error (error, g_steal_pointer (&child_error));
      return NULL;
...
gvfs_google_test_copy_file_from_one_parent_to_other_using_id (gconstpointer user_data)
{
  ...
  dest_file_display_name = get_file_attribute (dest_file, G_FILE_ATTRIBUTE_STANDARD_DISPLAY_NAME, &error);
  g_assert_no_error (error);
  g_assert_nonnull (dest_file_display_name);

  g_assert_cmpstr (dest_file_display_name, ==, source_file_title);
}
...
gvfs_google_test_move_file_from_one_parent_to_other_using_same_title (gconstpointer user_data)
{
  ...
  source_file = create_duplicate_file_with_random_title (self->test_dummy_file, &error);
  g_assert_no_error (error);
  g_assert_nonnull (source_file);
...
main (gint argc, gchar *argv [])
{
  ...
  // FIXME: Below case has some issue where segfault occurs in g_file_copy.
